...
gera_alt(B,I,G):-
    ...
    write('\nTEMPO: '),
    write(Time),
    write('\n\nCorrer AG novamente com as opções:\nRemover entrega: 1 \nAdicionar entrega: 2\nAlterar peso de entrega: 3\n'),
    read(Val),
...
gera_removerEntrega(B,I,G):-
    ...
    write('\n\nEntrega a remover (selecione o ID)'),
    read(OrderID),
    less_ind(L),
    delete_all(OrderID,L,V),
    write('\n\nV'),
    retractall(less_ind(_)),
...
gera_adicionarEntrega(B,I,G):-
    ...
    ((N>5,!,(N1 is N-6,remove_last_n(Val,N1,V));(V is Val))),
