...
bfs(Cam3):-
    retractall(tempo(_)),
    assertz(tempo(100000)),
    entrega_armazens(Ent),
    %entrega_armazens([_,_,B|_]),
    delete(Ent,5,Ent2),
    bfs2(Ent,[5|Ent2],[],Cam),
    append([5],Cam,Cam2),
    append(Cam2,[5],Cam3).
...
bfs2([Dest|L],[Act|Ent],LA,Cam):-
	findall([X|LA],
		(Dest\==Act,dadosCam_t_e_ta(_,Act,X,_,_,_),member(X,Ent)),Novos),
        tempo_act_x(Act,Novos,P),
        delete(Ent,P,Ent2),
        (   (P \== 0,!,append_inicio(P,LA,Todos)) ; (Todos = LA)),
	bfs2(L,[P|Ent2],Todos,Cam).

append_inicio(P,L,[P|L]).


tempo_act_x(_,[],0):-retract(tempo(_)),assertz(tempo(100000)).
tempo_act_x(Act,[[A|_]|Novos],X):-
    tempo_act_x(Act,Novos,X2),
