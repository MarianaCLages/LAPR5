...
O caso de uso em questão é a edição de um path. Este foge à regra dos outros casos de uso "clássicos", já que necessita de uma interface de forma a fazer a junção do módulo de gestão de armazéns e o módulo de logística.
...
Na vista de cenários apenas temos a interação desta UC que é a edição de um path.
Com edição o ator da UC vai poder tanto eliminar como editar o path.
...
, visto que no **SPRINT A**` does not existm atores explícitos) vai ter com o sistema para conseguir editar um path de forma correta.
Caso seja um ator, como por exemplo o ???, vai ter de ir introduzindo respetivamente as devidas informações até conseguir editar de forma correta o path (ou eliminar).
...
Na quarta alternativa funciona exatamente como a alt1 mas seria se fosse o caso de eliminar um path.

### Alternativa 5

![Nivel2-VP](VP_N1_alt5.svg)

Na quinta alternativa funciona exatamente como a alt2 mas seria se fosse o caso de eliminar um path.
...
Tal como explicado anteriormente na vista de processos de nível 1, vai existir VÁRIAS alternativas a mostrar TODAS as possibilidades de comunicação entre a aplicação (no caso de ser a edição de um path ou a eliminação do mesmo).
Neste caso concreto vai ser a edição de um path pelo o ??? (admin do sistema).
...
Na alternativa 4, é ilustrado o processo caso seja preciso deleter o path inves de editar. O ator do mesmo vai ser o ??? (administrador), possibilitando assim o despacho desejado (a eliminação em si).
...
No caso desta UC, edição de um path, quando o serviço recebe o pedido rencaminhado pela Route, procede a realizar o que é necessário, sendo este a edição do path recebido.
Primeiro, vai verificar se exists o path em questão, uma vez que é necessário saber se vamos editar algo que exists ou não.
Posteriormente, caso seja possível confirmar que exists de facto o path, vai verificar se os NOVOS WAREHOUSES, uma vez que é possível alterar tanto o destino como a origem de um path, se ambas existsm, pois não podemos ir para LOCALIDADES INEXISTSNTES.
Depois, finalmente, vai alterar, verificando também, tanto a energy que vai se gastar no path, o time de carregamento caso seja necessário, a distância de um armazém ao outro.
...
Na quarta alternativa o fluxo vai ser igual ao primeiro mas neste caso antes de alterar as informações vai deleter POR COMPLETO a entidade path em questão. Apenas vai receber o ID da mesma e vai verificar se exists, caso exista vai deleter o específico registo em consideração.
